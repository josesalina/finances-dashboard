...
import type { SnapshotDetail, SemaphoreRun, CurrentPrices, LiveHolding } from "../api/types";
...
export default function MonthReport() {
  ...
  // Fetch live prices and keep them only if this is the current snapshot
  useEffect(() => {
    const fetchLive = () => {
      api.get<CurrentPrices>("/snapshots/current-prices/")
        .then((r) => {
          if (String(r.data.snapshot_id) === id) {
            setLiveData(r.data);
          }
        })
        .catch(() => {});
    ...
    fetchLive();
    const interval = setInterval(fetchLive, LIVE_PRICES_INTERVAL_MS);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Link to="/dashboard" className="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 text-sm">← Dashboard</Link>
        <span className="text-gray-300 dark:text-gray-700">/</span>
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{snapshot.period}</h2>
        {semCode && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={c.label} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-5 py-4">
      ...
      {markowitzRaw && (
        <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 space-y-5">
          <div>
            <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Markowitz — Optimización de portafolio</p>
            <p className="text-xs text-gray-400 dark:text-gray-600 mt-0.5">
              Estrategia activa: <span className="text-gray-500 dark:text-gray-400">{PRIMARY_STRATEGY ?? "—"}</span>
            </p>
          </div>
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={weightsData} barCategoryGap="30%">
                    ...
                    <XAxis dataKey="symbol" tick={{ fill: chart.tick, fontSize: 11 }} />
                    <YAxis tick={{ fill: chart.tick, fontSize: 11 }} tickFormatter={(v) => `${v}%`} />
                    <Tooltip
                    ...
                    <div className="flex flex-col gap-1 flex-1 min-w-0">
                      {pieData.slice(0, 10).map((d, i) => (
                        <div key={d.name} className="flex items-center gap-1.5 text-xs">
                          ...
                          <span className="text-gray-500 dark:text-gray-400 font-mono w-10 shrink-0">{d.name}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-2 border-t border-gray-200 dark:border-gray-800">
            {/* Strategies */}
                  ...
                  {strategiesData.map((s) => (
                    <div
                      ...
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg text-xs ${
                        s.isPrimary ? "bg-green-900/20 border border-green-800/40" : "bg-gray-100/40 dark:bg-gray-800/40"
                      }`}
                    >
                      <div className="flex-1 min-w-0">
                        <p className={`font-medium truncate ${s.isPrimary ? "text-green-600 dark:text-green-400" : "text-gray-600 dark:text-gray-300"}`}>
                          {s.isPrimary && "★ "}{s.name}
                        </p>
                      </div>
                      <div className="flex gap-4 shrink-0 font-mono text-gray-500 dark:text-gray-400">
                        <span title="Retorno esperado">R: <span className="text-green-600 dark:text-green-400">{s.ret.toFixed(1)}%</span></span>
                        <span title="Volatilidad">σ: <span className="text-yellow-600 dark:text-yellow-400">{s.vol.toFixed(1)}%</span></span>
                        <span title="Sharpe">Sh: <span className="text-blue-600 dark:text-blue-400">{s.sharpe.toFixed(2)}</span></span>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    .map((r, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2 text-xs">
                          <span className="font-mono text-gray-500 dark:text-gray-400 w-10 shrink-0">{String(r["Símbolo"] ?? "—")}</span>
                          <div className="flex-1 bg-gray-200 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden">
                            <div
                              className={`h-full rounded-full ${sharpe >= 0 ? "bg-green-500" : "bg-red-500"}`}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                          <span className={`font-mono w-10 text-right ${sharpe >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                            {sharpe.toFixed(2)}
                          </span>
                        </div>
                      );
      ...
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Órdenes de rebalanceo</p>
        </div>
        {orders.length > 0 ? (
              ...
              {orders.map((o, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-gray-200/50 dark:border-gray-800/50">
                    <td className="px-4 py-3 font-mono font-medium text-gray-800 dark:text-gray-100">{String(o["Símbolo"] ?? "—")}</td>
                    <td className={`px-4 py-3 text-xs font-medium ${accion.includes("COMPR") ? "text-green-600 dark:text-green-400" : accion.includes("VEND") ? "text-red-600 dark:text-red-400" : "text-gray-500"}`}>
                      {accion}
                    </td>
                    <td className={`px-4 py-3 text-right font-mono text-sm ${delta >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                      {delta >= 0 ? "+" : ""}{fmt(delta)}
                    </td>
                  </tr>
                );
          ...
          <p className="px-5 py-8 text-sm text-gray-400 dark:text-gray-600 text-center">
      ...
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-5 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Semáforo de mercado</p>
          <button
            onClick={handleRefreshSemaforo}
            disabled={refreshing || !snapshot?.markowitz_raw}
            className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        ...
        {semCode && semInfo ? (
          <>
            ...
            <div className="flex items-start gap-4">
              <div className={`flex-1 flex items-center gap-3 p-3 rounded-lg border ${SEMAPHORE_COLORS[semCode] ?? ""}`}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">{String(semInfo.consejo ?? "")}</p>
                  {latestRun && (
                    <p className="text-xs text-gray-400 dark:text-gray-600 mt-0.5">
              ...
              {semPuntos != null && (
                <div className="text-center shrink-0 bg-gray-100 dark:bg-gray-800 rounded-xl px-4 py-3 min-w-[72px]">
                  <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Score</p>
                  <p className={`text-2xl font-bold font-mono ${
                    semPuntos <= 2 ? "text-green-600 dark:text-green-400" : semPuntos <= 6 ? "text-yellow-600 dark:text-yellow-400" : semPuntos <= 10 ? "text-orange-600 dark:text-orange-400" : "text-red-600 dark:text-red-400"
                  }`}>{semPuntos}</p>
            ...
            {/* VIX */}
            {semVix && (
              <div className="flex items-center gap-4 px-3 py-2 bg-gray-100/50 dark:bg-gray-800/50 rounded-lg text-xs">
                <span className="text-gray-500 uppercase tracking-wider">VIX</span>
                <span className="text-gray-700 dark:text-gray-200 font-mono font-bold text-base">{Number(semVix.valor ?? 0).toFixed(2)}</span>
                {semVix.vs_sma20 != null && (
                  <span className={`font-mono ${Number(semVix.vs_sma20) > 0 ? "text-red-500 dark:text-red-400" : "text-green-600 dark:text-green-500"}`}>
                  ...
                  {Object.entries(mercado).map(([ticker, data]) => {
                    ...
                    return (
                      <div key={ticker} className={`flex flex-col px-3 py-2 rounded-lg border text-xs font-mono min-w-[90px] ${
                        ...
                        <span className="text-gray-600 dark:text-gray-300 font-semibold text-[11px] mb-0.5">{ticker}</span>
                        <span className={r1 >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}>{r1 >= 0 ? "+" : ""}{r1.toFixed(2)}%</span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {/* Crédito */}
              {semCredito && (
                <div className="bg-gray-100/50 dark:bg-gray-800/50 rounded-lg p-3 space-y-1">
                  <p className="text-[10px] text-gray-500 uppercase tracking-wider">Crédito (HYG)</p>
                  <p className={`text-sm font-mono font-bold ${Number(semCredito.hyg_ret_1d ?? 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                  ...
                  {Boolean(semCredito.stress) && <p className="text-[10px] text-orange-500 dark:text-orange-400">⚠ Stress crediticio</p>}
                </div>
              )}
                  ...
                  {(["XLF", "XLU", "XLK"] as const).map((t) => {
                    ...
                    return (
                      <p key={t} className="text-[10px] font-mono">
                        ...
                        <span className={r >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}>{r >= 0 ? "+" : ""}{r.toFixed(2)}%</span>
                      </p>
                    );
                  })}
                  {Boolean(semSectores.rotacion_defensiva) && <p className="text-[10px] text-orange-500 dark:text-orange-400">⚠ Rotación defensiva</p>}
                  {Boolean(semSectores.stress_financiero) && <p className="text-[10px] text-red-600 dark:text-red-400">⚠ Financiero bajo presión</p>}
                </div>
              )}
              ...
              {semTasas && (
                <div className="bg-gray-100/50 dark:bg-gray-800/50 rounded-lg p-3 space-y-1">
                  ...
                  <p className="text-sm font-mono font-bold text-gray-700 dark:text-gray-200">{Number(semTasas.valor ?? 0).toFixed(3)}%</p>
                  <p className="text-[10px] text-gray-500">Δ5D: {(Number(semTasas.cambio_5d ?? 0)*100).toFixed(0)} bps</p>
                  {Boolean(semTasas.spike) && <p className="text-[10px] text-orange-500 dark:text-orange-400">⚠ Spike de tasas</p>}
                </div>
              )}
              ...
              {semEmergentes && (
                <div className="bg-gray-100/50 dark:bg-gray-800/50 rounded-lg p-3 space-y-1">
                  ...
                  <p className={`text-sm font-mono font-bold ${Number(semEmergentes.ret_1d ?? 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                  ...
                  {Boolean(semEmergentes.stress) && <p className="text-[10px] text-orange-500 dark:text-orange-400">⚠ Stress en emergentes</p>}
                </div>
              )}
            </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Holdings</p>
          {liveData && (
            <span className="text-xs text-gray-400 dark:text-gray-600">
              Live · {new Date(liveData.updated_at).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" })}
            </span>
          )}
        </div>
        <table className="w-full text-sm">
          ...
          <tbody>
            {snapshot.holdings
              ...
              .map((h) => {
                ...
                return (
                  <tr key={h.symbol} className="border-b border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-100/30 dark:hover:bg-gray-800/30">
                    <td className="px-5 py-3 font-mono font-medium text-gray-800 dark:text-gray-100">{h.symbol}</td>
                    <td className="px-5 py-3 text-right text-gray-500 dark:text-gray-400 font-mono">{Number(h.qty).toFixed(4)}</td>
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <span className="text-gray-600 dark:text-gray-300">{fmt(displayPrice)}</span>
                    ...
                    <td className="px-5 py-3 text-right text-gray-600 dark:text-gray-300">{Number(h.weight).toFixed(1)}%</td>
      ...
      {snapshot.dividends.length > 0 && (() => {
        ...
        return (
          <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Dividendos del período</p>
              <div className="flex items-center gap-4 text-xs">
                <span className="text-gray-500">Bruto: <span className="text-green-600 dark:text-green-400 font-mono">{fmt(totalGross)}</span></span>
                <span className="text-gray-500">Retención: <span className="text-red-600 dark:text-red-400 font-mono">{fmt(totalNet - totalGross)}</span></span>
                <span className="text-gray-500">Neto: <span className="text-gray-900 dark:text-white font-mono font-semibold">{fmt(totalNet)}</span></span>
              </div>
            </div>
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {[...gross, ...withholding]
                  ...
                  .map((d, i) => (
                    <tr key={i} className="border-b border-gray-200/40 dark:border-gray-800/40 hover:bg-gray-100/20 dark:hover:bg-gray-800/20">
                      <td className="px-5 py-2.5 text-gray-500 font-mono text-xs">{d.date}</td>
                      <td className="px-5 py-2.5 font-mono font-medium text-gray-800 dark:text-gray-100">{d.symbol || "—"}</td>
                      <td className="px-5 py-2.5 text-gray-500 text-xs">{d.event_type}</td>
                      <td className={`px-5 py-2.5 text-right font-mono ${Number(d.amount) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
      ...
      <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Transacciones del período</p>
        ...
        {snapshot.transactions.length > 0 ? (
                ...
                .map((t, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-gray-200/40 dark:border-gray-800/40 hover:bg-gray-100/20 dark:hover:bg-gray-800/20">
                      ...
                      <td className="px-5 py-2.5 font-mono font-medium text-gray-800 dark:text-gray-100">{t.symbol || "—"}</td>
                      <td className="px-5 py-2.5">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded ${isBuy ? "bg-green-500/10 text-green-600 dark:text-green-400" : "bg-red-500/10 text-red-600 dark:text-red-400"}`}>
                          {t.side || "—"}
                        </span>
                      </td>
                      <td className="px-5 py-2.5 text-right text-gray-500 dark:text-gray-400 font-mono">{Number(t.qty).toFixed(4)}</td>
                      <td className="px-5 py-2.5 text-right text-gray-600 dark:text-gray-300 font-mono">{fmt(t.price)}</td>
                      <td className={`px-5 py-2.5 text-right font-mono ${Number(t.amount) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
          ...
          <p className="px-5 py-8 text-sm text-gray-400 dark:text-gray-600 text-center">Sin transacciones registradas en este período.</p>
        )}
      </div>
