    ...
    <div className="p-6 text-gray-800 dark:text-gray-100 min-h-screen">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Config Dividendos</h1>
        ...
        {/* Error */}
        {error && (
          <div className="mb-4 px-4 py-3 bg-red-900/40 border border-red-700 rounded-lg text-red-600 dark:text-red-300 text-sm">
            {error}
          </div>
        )}
        ...
        {showForm && (
          <form
            ...
            className="mb-6 p-5 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl space-y-4"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="BMA"
                  className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white text-sm focus:outline-none focus:border-green-500"
                />
              </div>

              {/* Amount per share */}
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Div/Acción USD</label>
                <input
                  required
                  type="number"
                  step="0.000001"
              ...
              {/* Frecuencia */}
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Frecuencia</label>
                <select
              ...
              {/* Primer pago */}
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">Primer pago</label>
                <input
                  required
                  type="month"
              ...
              <div>
                <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                  Último pago <span className="text-gray-400 dark:text-gray-500">(opcional)</span>
                </label>
                <input
                  type="month"
                  value={form.end_date}
              ...
              <div className="flex items-center gap-3 pt-5">
                ...
                />
                <label htmlFor="tax_exempt" className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer select-none">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                type="button"
                onClick={cancelForm}
                className="px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 text-sm rounded-lg transition-colors"
              >
                Cancelar
              </button>
            </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                {configs.map((cfg) => (
                  <tr key={cfg.id} className="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="px-4 py-3 font-mono font-semibold text-green-600 dark:text-green-400">{cfg.symbol}</td>
                    <td className="px-4 py-3 text-gray-800 dark:text-white">${Number(cfg.amount_per_share).toFixed(6)}</td>
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-200">{FREQ_LABEL[cfg.interval_months] ?? `Cada ${cfg.interval_months}m`}</td>
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-200">{cfg.start_date?.slice(0, 7)}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{cfg.end_date ? cfg.end_date.slice(0, 7) : <span className="italic">indefinido</span>}</td>
                    <td className="px-4 py-3">
                      {cfg.tax_exempt
                        ? <span className="px-2 py-0.5 text-xs rounded-full bg-green-900/40 text-green-600 dark:text-green-400 border border-green-800">Exento</span>
                        : <span className="px-2 py-0.5 text-xs rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 border border-gray-300 dark:border-gray-700">30% NRA</span>}
                    </td>
                    <td className="px-4 py-3 text-gray-500 max-w-xs truncate">{cfg.notes || "—"}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button
                          onClick={() => openEdit(cfg)}
                          className="px-2 py-1 text-xs bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 rounded transition-colors"
                        >
                          Editar
                        </button>
                        <button
                          onClick={() => handleDelete(cfg)}
                          className="px-2 py-1 text-xs bg-red-900/50 hover:bg-red-800 text-red-600 dark:text-red-300 rounded transition-colors"
                        >
                          Eliminar
                        </button>
                      </div>
